            ...
            try {
                ...
                for (auto&& obj : util::get_list_of_replicas_to_trim(*input, repl_list)) {
                    rodsLog(LOG_DEBUG, "Replica to trim [data_object=%s, replica_number=%d, physical_path=%s]",
                                       input->objPath, obj.repl_num(), obj.path().c_str());
                    ...
                    if (const auto object = util::find_hard_link(hl_info, std::to_string(obj.resc_id())); object) {
                        ...
                        rodsLog(LOG_DEBUG, "Unregistering replica [UUID=%s, resource_id=%s] ...", hl.uuid.data(), hl.resource_id.data());

                        if (const auto ec = util::unregister_replica(conn, input->objPath, std::to_string(obj.repl_num())); ec < 0) {
                            rodsLog(LOG_ERROR, "Could not unregister replica [data_object=%s, replica_number=%d]", input->objPath, obj.repl_num());
                            return ERROR(ec, "Could not unregister replica");
                        }
                        ...
                        try {
                            ...
                            rodsLog(LOG_ERROR, "Could not remove hard link metadata "
                                               ...
                                               e.code().value(), e.what(), input->objPath, obj.repl_num(), hl.uuid.data(), hl.resource_id.data());
                            return ERROR(e.code().value(), e.what());
                        }
                    }
                    else {
